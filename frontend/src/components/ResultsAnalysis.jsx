...
export default function ResultsAnalysis({ poll, pollId, onClose }) {
  ...
  const handleExportPdf = () => {
    ...
    doc.autoTable({
      head: [['#', 'Kullanıcı', 'Puan', 'Süre (ms)']],
      body: scores.map((s, i) => [i + 1, s.userName, s.points, s.totalTimeMs]),
      startY: 24,
    });
    doc.save(`${poll.title || 'rapor'}.pdf`);
  };

  const handleExportExcel = () => {
    const data = [
      ['Kullanıcı', 'Puan', 'Süre (ms)'],
      ...scores.map((s) => [s.userName, s.points, s.totalTimeMs]),
    ];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(data), 'Skorlar');
    XLSX.writeFile(wb, `${poll.title || 'rapor'}.xlsx`);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-50 overflow-y-auto">
      <header className="bg-slate-900 text-white p-4 flex items-center justify-between sticky top-0 z-10">
        <h2 className="font-bold text-lg">📊 {poll.title} — Sonuç Analizi</h2>
        <div className="flex gap-2">
          ...
          <button onClick={onClose} className="px-3 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm">Kapat</button>
        </div>
      </header>

      <div className="p-6 max-w-5xl mx-auto">
        <div className="flex gap-2 mb-4">
          <button onClick={() => setActiveTab('overview')} className={`px-4 py-2 rounded-lg text-sm ${activeTab === 'overview' ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-200'}`}><BarChart3 size={16} className="inline mr-1" /> Sorular</button>
          {typeConfig.hasCorrectAnswer && (
            <button onClick={() => setActiveTab('leaderboard')} className={`px-4 py-2 rounded-lg text-sm ${activeTab === 'leaderboard' ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-200'}`}><Trophy size={16} className="inline mr-1" /> Liderlik</button>
          )}
        </div>
        ...
        {analytics && (
          <div className="bg-white rounded-xl p-4 border border-slate-200 mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500">Toplam katılımcı</div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            {analytics.questionAnalysis.map((qa, i) => (
              <div key={i} className="bg-white rounded-xl p-4 border border-slate-200">
                <h3 className="font-bold mb-3">Soru {i + 1}: {qa.question}</h3>
                {qa.options.length > 0 ? (
                  ...
                  <p className="text-slate-400 italic text-sm">Bu soru çoktan seçmeli değil.</p>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 divide-y">
            {scores.map((s, i) => (
              <div key={s.userName} className="p-3 flex items-center justify-between">
                ...
                <span className="font-bold text-indigo-600">{s.points} puan</span>
              </div>
            ))}
            {scores.length === 0 && <p className="p-4 text-slate-400 text-center">Henüz skor yok.</p>}
          </div>
        )}

        {openAnswers.length > 0 && (
          <div className="mt-6 bg-white rounded-xl p-4 border border-slate-200">
            <h3 className="font-bold mb-3">✏️ Açık Uçlu Cevaplar ({openAnswers.length})</h3>
